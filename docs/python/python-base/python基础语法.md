...
访问元组，元组既可以通过下标索引来访问，也可以通过切片来访问，注意切片返回的事一个包含切片片段的新元组
...
与列表方法（切片，获取长度，获取下标对应的值）没有什么区别，唯一不同的是元组不能修改元素的值，所以当需要修改元素的值的时候可以把元组转为列表来操作
...
```python
# 字符串拼接,注意最后一个元素，这次我们要将它设置成字符串，因为join函数要求参数序列中的每一个元素都必须是字符串
w = ('My', 'age', 'is', '18')
# 输出My age is 18
print(' '.join(w))
```
...
# 查找,输出查到的元素所在的下标位置
print(w1.index('b'))
# 如果查找的元素不存在，则打印错误信息：ValueError: tuple.index(x): x not in tuple
print(w1.index('c'))
```
